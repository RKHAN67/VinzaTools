$out = 'D:\Projects\toolkit-pro\testing\server-media.out.log'
$err = 'D:\Projects\toolkit-pro\testing\server-media.err.log'
...
try {
  ...
  if ($healthOk) {
    $env:MEDIA_SMOKE_API_BASE = $existingBase
  } else {
    ...
    $env:MEDIA_SMOKE_API_BASE = "http://127.0.0.1:$port"
  }

  python '.\testing\run_media_smoke_tests.py'
}
finally {
  Remove-Item Env:\MEDIA_SMOKE_API_BASE -ErrorAction SilentlyContinue
